...
namespace Integration.Tests.Helpers
{
    public static class Seed
    {
        ...
        /// <param name="minPlayerId">Minimum Player Id</param>
        /// <param name="maxPlayerId">Maximum Player Id</param>
        /// <param name="maxParallelism">Max level of parallelism.</param>
        /// <returns></returns>
        public static async Task<List<GetMatchResult>> SeedMatches(this IMatchApi match, int count, int minPlayerId, int maxPlayerId, int maxParallelism = 250)
        {
            if (count < maxParallelism)
                maxParallelism = count;

            var generator = DataGenerators.Match.GetMatchCommand(minPlayerId, maxPlayerId);
            var results   = new List<GetMatchResult>(count);
            var tasks     = new Task<ApiResponse<GetMatchResult>>[maxParallelism];
            var matches   = Mapping.Mapper.Map<List<PostMatchRequest>>(generator.Generate(count));
